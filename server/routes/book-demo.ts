...
export const bookDemo: RequestHandler = async (req, res) => {
  try {
    const { name, email, phone, company, message, selectedDate, selectedTime }: BookDemoRequest = req.body;

    // Validate required fields
    if (!name || !email || !selectedDate || selectedTime === undefined) {
      return res.status(400).json({ 
        success: false, 
        message: 'Missing required fields: name, email, selectedDate, and selectedTime' 
      } as BookDemoResponse);
    }
    ...
    const appointmentDateTime = new Date(`${selectedDate}T${selectedTime + 8}:00:00.000Z`); // Pacific time + 8 hours = UTC
